...
export default function StaffingPage() {
  ...
  const input: StaffingInput = { volume, aht, slaTarget, timeTarget, occupancyTarget, shrinkage, currentStaff: defaults.staff };
  const result = useMemo(() => calculateStaffing(input), [volume, aht, slaTarget, timeTarget, occupancyTarget, shrinkage, defaults.staff]);
  ...
  return (
    <div className="min-h-screen bg-background bg-dot-pattern p-6 lg:p-8">
      ...
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        ...
        <p className="text-sm text-muted-foreground ml-11">Calcula el staff necesario para cumplir tus objetivos de SLA con Erlang C</p>
      </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12 }} className="bg-card rounded-xl border border-border p-5">
          <div className="flex items-center gap-1.5">
            ...
            <Tooltip>
              ...
              <TooltipContent className="max-w-[200px] text-xs">Nivel de servicio calculado con el modelo Erlang C</TooltipContent>
            </Tooltip>
          </div>
